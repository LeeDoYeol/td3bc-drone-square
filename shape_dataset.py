# NOTE: 이 파일은 동료 저장소 subsubli/drone_simulation 의 데이터 생성 시뮬레이터입니다.
# 데이터셋을 만든 것과 동일한 시뮬레이터로 학습 정책을 평가(sim_eval.py)하기 위해 포함했습니다.
...
SHAPE_SIDES = {'line': 2, 'triangle': 3, 'square': 4, 'pentagon': 5, 'star': 10, 'circle': 72}
...
def generate_local_shape_waypoints(shape, num_wp, radius, side_jitter, rng):
    ...
    if shape == 'circle':
        ...
        #### 별: 바깥/안쪽 반지름을 번갈아 둔 10개 꼭짓점(각도 오름차순 -> 단순 다각형인 별 외곽선).
        n_sides = SHAPE_SIDES['star']
        ratios = np.tile([1.0, 0.4], n_sides // 2)  # 바깥, 안쪽 번갈아
        vertex_radii = radius * ratios * (1 + rng.uniform(-side_jitter, side_jitter, size=n_sides))
    else:
        #### 'line'(2) 은 여기서 처리됨: 꼭짓점 2개 -> 닫으면 A->B->A 왕복 직선.
